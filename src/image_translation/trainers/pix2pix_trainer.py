...
class Pix2PixTrainer():
    ...
    def __init__(self, opt, resume_epoch=0):
        ...
        # use EMA(exponential moving average) in G.
        if opt.use_ema:
    ...
    def run_generator_one_step(self, data, alpha=1):
        ...
        # get many G-loss and compute a mean.
        g_losses, out = self.pix2pix_model(data, mode='generator', alpha=alpha)
        # add a new function to compute consistency loss.
        
        # if self.opt.multi_view_consistency_loss:
        #     KLloss = torch.nn.KLDivLoss()
        #     # get a union group.
        #     part_have = pre_out['part_have'].cpu().numpy()
        #     part2img = {}
        #     for j in range(58):
        #         part2img[j] = []
        #         for k in range(4):
        ...
        #     klloss_img = {}
            ...
            #     # for different parts.
            #     for k in range(58):
            #         # if this part exists in 1 img and exits in other imgs.
            #         if part_have[j][k] > 0 and len(part2img[k]) > 1:
            #             kllosses[k] = {}
            #             for l in range(len(part2img[k])):
            #                 if part2img[k][l] != j:
            #                     # print(pre_out['material'][j][k])
            #                     # print(pre_out['material'][part2img[k][l]][k])
            #                     ori_mat = pre_out['material'][j][k]
            #                     tar_mat = pre_out['material'][part2img[k][l]][k]
            #                     kllosses[k][part2img[k][l]] = KLloss(ori_mat.log(), tar_mat)
                        
            #             klloss_part[k] = sum(kllosses[k].values()).mean()                       
            #     klloss_img[j] = sum(klloss_part.values()).mean()
            
            # # transform shape.
            # consis1 = torch.cat([klloss_img[0].unsqueeze(dim=0), klloss_img[1].unsqueeze(dim=0)])
            # consis2 = torch.cat([klloss_img[2].unsqueeze(dim=0), klloss_img[3].unsqueeze(dim=0)])
            # g_losses['consis'] = torch.cat([consis1, consis2]) * self.opt.weight_multiview
    ...
    def save(self, epoch):
        ...
        torch.save({'G': self.optimizer_G.state_dict(),
                        ...
                        'lr':  self.old_lr,
                        }, os.path.join(self.opt.checkpoints_dir, self.opt.name, str(epoch) + '_optimizer.pth'))
        if self.opt.use_ema:
