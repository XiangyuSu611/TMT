...
class RenderedDataset(Pix2pixDataset):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.set_defaults(label_nc=57)
    ...
    def get_paths(self, opt):
        ...
        for p in all_images:
            ...
            elif p.endswith('.png') and not p.endswith('_shapenet.png') and not p.endswith('_vis.png'):
                label_paths.append(p)
            elif p.endswith('.json'):
                materialGT_paths.append(p)
        return label_paths, image_paths, materialGT_paths
        
    def get_ref(self, opt):
        extra = '_test' if opt.phase == 'test' else ''
        with open(opt.dataroot + '/ref{}.txt'.format(extra)) as fd:
            lines = fd.readlines()
        ref_dict = {}
