...
def is_image_file(filename):
    return any(filename.endswith(extension) for extension in IMG_JSON_EXTENSIONS)


def make_dataset_rec(dir, images):
    assert os.path.isdir(dir), '%s is not a valid directory' % dir
    ref_name = 'ref.txt' if 'training' in dir else 'ref_test.txt'
    with open(Path(Path(dir).parent, ref_name), 'r') as f:
        lines = f.readlines()
        num_txt = len(lines)
