...
class Pix2pixDataset(BaseDataset):
    ...
    def initialize(self, opt):
        self.opt = opt
        label_paths, image_paths, materialGT_paths = self.get_paths(opt)
        ...
        materialGT_paths = materialGT_paths[:opt.max_dataset_size]

        if not opt.no_pairing_check:
            for path1, path2, path3 in zip(label_paths, image_paths, materialGT_paths):
                assert self.paths_match(path1, path2) and self.paths_match(path1, path3), \
                    "The label-image pair (%s, %s, %s) do not look like the right pair because the filenames are quite different. Are you sure about the pairing? Please see data/pix2pix_dataset.py to see what is going on, and use --no_pairing_check to bypass this." % (path1, path2, path3)

        self.label_paths = label_paths
        self.image_paths = image_paths
        self.materialGT_paths = materialGT_paths
    ...
    def get_paths(self, opt):
        ...
        return label_paths, image_paths, materialGT_paths
    ...
    def __getitem__(self, index):
        ...
        if random_p < self.real_reference_probability or self.opt.phase == 'test':
            ...
            if 'png' in list(self.ref_dict.keys())[0]:
            ...
            if self.opt.dataset_mode == 'deepfashion':
                ...
                if 'val' in path_ref:
                    path_ref = os.path.dirname(image_path) + '/' + path_ref
                else:
            ...
            # material_GT.
            mat_path = path_ref.replace('.jpg', '.json')
        else:
            pair = False
        ...
        # additional material gt tensor.
        mat_arr = np.zeros([57,3], dtype=float) # shape: [label_nc, 3] 
        mat_ref_tensor = torch.from_numpy(mat_arr)
        input_dict = {'label': label_tensor,
                        ...
                        'materialGT_tensor':mat_ref_tensor,
                    }
