...
class Pix2PixModel(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        # set loss functions
        if opt.isTrain:
            # vgg for image feature extract.
            self.vggnet_fix = networks.correspondence.VGG19_feature_color_torchversion(vgg_normal_correct=opt.vgg_normal_correct)
            self.vggnet_fix.load_state_dict(torch.load('src/image_translation/models/vgg19_conv.pth'))
            self.vggnet_fix.eval()
            for param in self.vggnet_fix.parameters():
                param.requires_grad = False
            self.vggnet_fix.to(self.opt.gpu_ids[0])
            # vgg for mask feature extract.
            self.maskvggnet_fix = networks.correspondence.VGG19_feature_mask_torchversion()
            # change state dict key words to reload.
            ori_dict = torch.load('src/image_translation/models/vgg19_mask_conv2.pth')['state_dict']
            corr_dict = {}
            tar_dict = {}
            corr_dict['0'] = '1_1'
            corr_dict['2'] = '1_2'
            ...
            corr_dict['32'] = '5_3'
            corr_dict['34'] = '5_4'
            for param in ori_dict:
                if param.startswith('feature'):
                    temp_type = param[param.rfind('.') + 1:]
                    temp = param[:param.rfind('.')]
                    index = temp[temp.rfind('.') + 1:]
                    if index in corr_dict:
                        tar_index = corr_dict[index]
                        tar_dict['conv' + corr_dict[index] + '.' + temp_type] = ori_dict[param]
            self.maskvggnet_fix.load_state_dict(tar_dict)
            self.maskvggnet_fix.eval()
            for param in self.vggnet_fix.parameters():
                param.requires_grad = False
            self.vggnet_fix.to(self.opt.gpu_ids[0])
    ...
    def forward(self, data, mode, GforD=None, alpha=1):
        input_label, input_semantics, real_image, self_ref, ref_image, ref_label, ref_semantics, materialsGT_ref, id2matDis = self.preprocess_input(data, )
        self.alpha = alpha
        generated_out = {}
        if mode == 'generator':
            g_loss, generated_out = self.compute_generator_loss(input_label, 
                input_semantics, real_image, materialsGT_ref, id2matDis, ref_label, ref_semantics, ref_image, self_ref)
            out = {}
            ...
            out['warp_mask_to_ref'] = None if 'warp_mask_to_ref' not in generated_out else generated_out['warp_mask_to_ref']
    ...
    def initialize_networks(self, opt):
        ...
        # load predictor.
        if opt.isTrain and self.opt.predictor_loss:
            check_point_path = Path('/mnt_new/xiangyu/code/CoCosNet-master/pho_predictor/checkpoint/1107/ori_model_best.pth.tar')
            ...
            net['predictor'] =  predictor if opt.predictor_loss else None
    ...
    def preprocess_input(self, data):
        ...
        if self.use_gpu():
            ...
            data['materialsGt_ref'] = data['materialGT_tensor'].cuda()
            if self.opt.dataset_mode != 'deepfashion':
        ...
        id2matDis = {}
        return data['label'], input_semantics, data['image'], data['self_ref'], data['ref'], data['label_ref'], ref_semantics, data['materialsGt_ref'], id2matDis
    ...
    def compute_generator_loss(self, input_label, input_semantics, real_image, materialsGT_ref, id2matDis, ref_label=None, ref_semantics=None, ref_image=None, self_ref=None):
        G_losses = {}
        ...
        # use D get fake or real probility.
        pred_fake, pred_real, seg, fake_cam_logit, real_cam_logit = self.discriminate(input_semantics, generate_out['fake_image'], real_image)
        # get GAN loss.
        ...
        # mask features.
        warp_mask_to_ref_256 = F.interpolate(generate_out['warp_mask_to_ref'], size=[256,256], mode='bilinear')
        zero = torch.zeros_like(warp_mask_to_ref_256)
        warp_mask_to_ref_domain = torch.where(ref_label > 0, warp_mask_to_ref_256, zero)
        warp_mask_256 = F.interpolate(generate_out['warp_mask'], size=[256,256], mode='bilinear')
        warp_mask_256_domain = torch.where(input_label > 0, warp_mask_256, zero)
        
        fake_ref_mask_features = self.maskvggnet_fix(warp_mask_to_ref_256,['r12', 'r22'], preprocess=False)
        fake_input_mask_features = self.maskvggnet_fix(warp_mask_256,['r12', 'r22'], preprocess=False)
        ...
        if self.opt.warp_mask_losstype != 'none':
            ...
            ref_label = ref_label.long().squeeze(1)
            gt_label = input_label.long().squeeze(1)
            weights = []
            weights_ref = []
            ...
            G_losses['mask'] = (F.nll_loss(torch.log(warp_mask_256 + 1e-10), gt_label, reduce =False) * weights).sum() / (weights.sum() + 1e-5) * self.opt.weight_mask
            if self.opt.warp_mask_losstype == 'direct':
                for i in range(ref_label.shape[0]):
                    ref_label_uniq = torch.unique(ref_label[i])
                    gt_label_uniq = torch.unique(gt_label[i])      
                    zero_label_ref = [it for it in ref_label_uniq if it not in gt_label_uniq]
                    weight_ref = torch.ones_like(gt_label[i]).float()
                    for r in zero_label_ref:
                        weight_ref[ref_label[i] == r] = 0
                    weight_ref[ref_label[i] == 0] = 0
                    weights_ref.append(weight_ref.unsqueeze(0))
                weights_ref = torch.cat(weights_ref, dim=0)           
                # G_losses['mask'] += (F.nll_loss(torch.log(generate_out['warp_mask_to_ref'] + 1e-10), ref_label, reduce =False) * weights_ref).sum() / (weights_ref.sum() + 1e-5) * self.opt.weight_mask
                G_losses['mask'] += (F.nll_loss(torch.log(warp_mask_to_ref_256 + 1e-10), ref_label, reduce =False) * weights_ref).sum() / (weights_ref.sum() + 1e-5) * self.opt.weight_mask
    ...
    def generate_fake(self, input_semantics, real_image, ref_semantics=None, ref_image=None, self_ref=None):
        ...
        generate_out['ref_mask_features'] = self.maskvggnet_fix(ref_semantics, ['r12', 'r22', 'r32', 'r42', 'r52'], preprocess=False)
        generate_out['input_mask_features'] = self.maskvggnet_fix(input_semantics, ['r12', 'r22', 'r32', 'r42', 'r52'], preprocess=False)
        # Cross-Iteration Batch Normalizartion(CBN).
        # Another type of Batch Normalization.
    ...
    def inference(self, input_semantics, ref_semantics=None, ref_image=None, self_ref=None):
        ...
        if self.opt.CBN_intype == 'mask':
            ...
            CBN_in = torch.cat((coor_out['warp_out'], input_semantics), dim=1)
        generate_out['fake_image'] = self.net['netG'](input_semantics, warp_out=CBN_in)
        #generate_out['cyc_semantic'] = coor_out['cyc_semantic']
        generate_out = {**generate_out, **coor_out}
        return generate_out
