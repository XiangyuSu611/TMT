...
#################################################################################################
# 20201213 get a new corr Matric.
        phi_permute = phi.permute(0, 2, 1)
        f_permute = torch.matmul(phi_permute, theta)
        if detach_flag:
            f_permute = f_permute.detach
        if WTA_scale_weight == 1:
            f_permute_WTA = f_permute
        else:
            f_permute_WTA = WTA_scale.apply(f_permute, WTA_scale_weight)
        f_permute_WTA = f_permute_WTA / temperature
        if return_corr:
            return f_permute_WTA
        f_permute_div_C = F.softmax(f_permute_WTA.squeeze(), dim=-1)
        

        seg = F.avg_pool2d(seg_map, self.opt.down)
        channel_seg = seg.shape[1]
        seg = seg.view(batch_size, channel_seg, -1)
        seg = seg.permute(0, 2, 1)
        y_permute = torch.matmul(f_permute_div_C,seg)
        print(y_permute.shape)

        semantic = {}
        for i in range(5):
            semantic_part = y_permute[:,:,i]
            semantic_part = semantic_part.view(batch_size, 64, -1)
            semantic_part = semantic_part.view(batch_size, 1, 64, -1)
            semantic_part = F.interpolate(semantic_part, size=[500,500], mode='bilinear')
            semantic_part = semantic_part.squeeze()
            # T.
            semantic[i] = semantic_part

        cyc_semantic = torch.stack((semantic[0], semantic[1], semantic[2], semantic[3], semantic[4]),dim=3)
        coor_out['cyc_semantic'] = cyc_semantic
#########################################################################################################        
        ...
        # # downsample the seg_map
        # seg = F.avg_pool2d(seg_map, self.opt.down)
        # channel_seg = seg.shape[1]
        # seg = seg.view(batch_size, channel_seg, -1)
        # seg = seg.permute(0, 2, 1)
        # f_div_D = F.softmax(f_WTA.squeeze(), dim=1)
        # seg_y = torch.matmul(f_div_D, seg)
